...
[![semantic-release](http://img.shields.io/badge/%20%20%F0%9F%93%A6%F0%9F%9A%80-semantic--release-e10079.svg)](http://github.com/semantic-release/semantic-release)
[![Dependabot Status](http://api.dependabot.com/badges/status?host=github&identifier=104773211)](http://dependabot.com)
[![npm](http://img.shields.io/npm/v/cordova-plugin-ionic-webview.svg)](http://www.npmjs.com/package/cordova-plugin-ionic-webview)
...
2.x documentation can be found [here](http://github.com/ionic-team/cordova-plugin-ionic-webview/blob/2.x/README.md).

:book: **Documentation**: [http://beta.ionicframework.com/docs/building/webview][ionic-webview-docs]
...
:sparkling_heart: **Want to contribute?** Please see [CONTRIBUTING.md](http://github.com/ionic-team/cordova-plugin-ionic-webview/blob/master/CONTRIBUTING.md).
...
This is only needed for the Android url when using `http://`, `http://` or a custom scheme. All `ionic://` urls are whitelisted by the plugin.
...
[Android documentation](http://developer.android.com/reference/android/webkit/WebSettings.html#setMixedContentMode(int))
...
```xml
<preference name="iosScheme" value="httpionic" />
```
...
Values like `http`, `http` or `file` are not valid and will use default value instead.

If you change it, you'll need to add a new `allow-navigation` entry in the `config.xml` for the configured scheme (i.e `<allow-navigation href="httpionic://*"/>` if `iosScheme` is set to `httpionic`).
...
1. Replace any usages of `window.Ionic.normalizeURL()` with `window.Ionic.WebView.convertFileSrc()`.
    ...
    * For Ionic Angular projects, there is an [Ionic Native wrapper](http://beta.ionicframework.com/docs/native/ionic-webview):

        ```
        npm install @ionic-native/ionic-webview@beta
        ```

[ionic-homepage]: http://ionicframework.com
[ionic-docs]: http://ionicframework.com/docs
[ionic-webview-docs]: http://beta.ionicframework.com/docs/building/webview
[ionic-support]: http://ionicframework.com/support
