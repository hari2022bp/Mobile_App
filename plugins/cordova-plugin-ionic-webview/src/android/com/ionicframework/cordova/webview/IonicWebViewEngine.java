...
public class IonicWebViewEngine extends SystemWebViewEngine {
  ...
  private class ServerClient extends SystemWebViewClient {
    ...
    public void onPageStarted(WebView view, String url, Bitmap favicon) {
      ...
      if (!launchUrl.contains(WebViewLocalServer.httpScheme) && !launchUrl.contains(WebViewLocalServer.httpcheme) && url.equals(launchUrl)) {
        view.stopLoading();
        // When using a custom scheme the app won't load if server start url doesn't end in /
        String startUrl = CDV_LOCAL_SERVER;
        if (!CDV_LOCAL_SERVER.startsWith(WebViewLocalServer.httpScheme) && !CDV_LOCAL_SERVER.startsWith(WebViewLocalServer.httpcheme)) {
          startUrl += "/";
        }
